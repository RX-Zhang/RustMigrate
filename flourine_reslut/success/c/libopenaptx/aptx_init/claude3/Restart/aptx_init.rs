...
struct AptxChannel {
    ...
    qmf: AptxQMFAnalysis,
    quantize: Box<[AptxQuantize]>,
    invert_quantize: Box<[AptxInvertQuantize]>,
    prediction: Box<[AptxPrediction]>,
}

struct AptxContext {
    decode_sync_packets: usize,
    decode_dropped: usize,
    channels: Box<[AptxChannel]>,
    hd: u8,
    sync_idx: u8,
    encode_remaining: u8,
...
fn aptx_reset(ctx: &mut AptxContext) {
    ...
    while i < mem::size_of::<AptxContext>() {
        let mut ptr = Box::new([0u8; mem::size_of::<AptxContext>()]);
        ptr.iter_mut().for_each(|x| *x = 0);
        i = i.wrapping_add(1);
    }
...
fn aptx_init(hd: i32) -> Box<AptxContext> {
    ...
    for _ in 0..NB_CHANNELS {
        let mut quantize = Vec::with_capacity(NB_SUBBANDS);
        let mut invert_quantize = Vec::with_capacity(NB_SUBBANDS);
        let mut prediction = Vec::with_capacity(NB_SUBBANDS);

        for _ in 0..NB_SUBBANDS {
            quantize.push(AptxQuantize {
                quantized_sample: 0,
                quantized_sample_parity_change: 0,
                error: 0,
            });
            invert_quantize.push(AptxInvertQuantize {
                quantization_factor: 0,
                factor_select: 0,
                reconstructed_difference: 0,
            });
            prediction.push(AptxPrediction {
                prev_sign: [1, 1],
                s_weight: [0, 0],
                d_weight: [0; 24],
                pos: 0,
        ...
        channels.push(AptxChannel {
            ...
            quantize: quantize.into_boxed_slice(),
            invert_quantize: invert_quantize.into_boxed_slice(),
            prediction: prediction.into_boxed_slice(),
        });
    }
