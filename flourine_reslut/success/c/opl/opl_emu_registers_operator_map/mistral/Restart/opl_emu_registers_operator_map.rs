...
fn opl_emu_registers_byte(regs: &OplEmuRegisters, offset: u32, start: u32, count: u32, extra_offset: u32) -> u32 {
    opl_emu_bitfield(u32::from_le_bytes(regs.m_regdata[offset as usize + extra_offset as usize..offset as usize + extra_offset as usize + mem::size_of::<u32>()].try_into().unwrap()), start, count)
}
...
fn opl_emu_registers_operator_map(regs: &OplEmuRegisters, dest: &mut OplEmuRegistersOperatorMapping) {
    ...
    dest.chan[9] =
        if opl_emu_bitfield(fourop, 3, 1) != 0 { opl_emu_registers_operator_list(18, 21, 24, 27) } else {
            opl_emu_registers_operator_list(18, 21, 0xff, 0xff)
        };
    dest.chan[10] =
        if opl_emu_bitfield(fourop, 4, 1) != 0 { opl_emu_registers_operator_list(19, 22, 25, 28) } else {
            opl_emu_registers_operator_list(19, 22, 0xff, 0xff)
        };
    dest.chan[11] =
        if opl_emu_bitfield(fourop, 5, 1) != 0 { opl_emu_registers_operator_list(20, 23, 26, 29) } else {
            opl_emu_registers_operator_list(20, 23, 0xff, 0xff)
        };
    dest.chan[12] =
        if opl_emu_bitfield(fourop, 3, 1) != 0 { opl_emu_registers_operator_list(0xff, 0xff, 0xff, 0xff) } else {
            opl_emu_registers_operator_list(24, 27, 0xff, 0xff)
        };
    dest.chan[13] =
        if opl_emu_bitfield(fourop, 4, 1) != 0 { opl_emu_registers_operator_list(0xff, 0xff, 0xff, 0xff) } else {
            opl_emu_registers_operator_list(25, 28, 0xff, 0xff)
        };
    dest.chan[14] =
        if opl_emu_bitfield(fourop, 5, 1) != 0 { opl_emu_registers_operator_list(0xff, 0xff, 0xff, 0xff) } else {
