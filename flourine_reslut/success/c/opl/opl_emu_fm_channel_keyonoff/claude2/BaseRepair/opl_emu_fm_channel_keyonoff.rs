...
fn opl_emu_fm_operator_keyonoff(fmop: &mut OplEmuFmOperator, on: u32, op_type: OplEmuKeyonType) {
    fmop.m_keyon_live = fmop.m_keyon_live.wrapping_add((opl_emu_bitfield(on, 0, 1) << op_type as u32) as u8); 
}
...
fn opl_emu_fm_channel_keyonoff(fmch: &mut OplEmuFmChannel, states: u32, op_type: OplEmuKeyonType, chnum: u32) {
    for opnum in 0..fmch.m_op.len() {
        if let Some(ref mut fmop) = fmch.m_op[opnum] {
             opl_emu_fm_operator_keyonoff(fmop, opl_emu_bitfield(states, opnum as i32, 1), op_type);
        }
    }
}
