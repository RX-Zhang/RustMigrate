...
fn opl_emu_fm_operator_keyonoff(fmop: &mut OplEmuFmOperator, on: u32, keyon_type: OplEmuKeyonType) {
    ...
    fmop.m_keyon_live = fmop.m_keyon_live.wrapping_sub(prev_bit)
        .wrapping_add((bit << (keyon_type as u32)).try_into().unwrap());
}

fn opl_emu_fm_channel_keyonoff(fmch: &mut OplEmuFmChannel, states: u32, keyon_type: OplEmuKeyonType, chnum: u32) {
    for opnum in 0..fmch.m_op.len() {
        if let Some(ref mut fmop) = fmch.m_op[opnum] {
             opl_emu_fm_operator_keyonoff(fmop, opl_emu_bitfield(states, opnum as i32, 1), keyon_type);
        }
    }
}
