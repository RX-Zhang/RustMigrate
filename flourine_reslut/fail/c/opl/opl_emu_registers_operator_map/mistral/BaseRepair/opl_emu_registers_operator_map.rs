...
fn opl_emu_registers_byte(regs: &OplEmuRegisters, offset: u32, start: u32, count: u32, extra_offset: u32) -> u32 {
    opl_emu_bitfield(u32::from_le_bytes(regs.m_regdata[offset as usize + extra_offset as usize..offset as usize + (extra_offset + 1) as usize].try_into().unwrap()), start, count)
}
