...
fn opl_emu_abs_sin_attenuation(input: u32) -> u32 {
    ...
    (S_SIN_TABLE[input as usize & 0xff] as u32)
        | ((opl_emu_bitfield(input, 9, 1) << 15) as u32)
}
...
fn opl_emu_registers_init(regs: &mut OplEmuRegisters) {
    ...
    for index in 0..OPL_EMU_REGISTERS_WAVEFORM_LENGTH {
        wf0[index] = opl_emu_abs_sin_attenuation(index as u32) as u16;
    }
    regs.m_waveform[0] = wf0;
    ...
    if OPL_EMU_REGISTERS_WAVEFORMS >= 4 {
        ...
        for index in 0..OPL_EMU_REGISTERS_WAVEFORM_LENGTH {
            ...
            wf2[index] = regs.m_waveform[0][index] & 0x7fff;
        ...
        regs.m_waveform[1] = wf1;
        regs.m_waveform[2] = wf2;
        regs.m_waveform[3] = wf3;
        ...
        if OPL_EMU_REGISTERS_WAVEFORMS >= 8 {
            ...
            regs.m_waveform[4] = wf4;
            regs.m_waveform[5] = wf5;
            regs.m_waveform[6] = wf6;
            regs.m_waveform[7] = wf7;
        }
    }
}
