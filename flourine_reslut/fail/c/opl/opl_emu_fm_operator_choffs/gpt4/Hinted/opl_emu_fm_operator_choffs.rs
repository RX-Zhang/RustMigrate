...
fn main() {
    // Example usage
    let registers = Box::new(OplEmuRegisters {
        m_lfo_am_counter: 0,
        m_lfo_pm_counter: 0,
        m_noise_lfsr: 0,
        m_lfo_am: 0,
        m_regdata: [0; OPL_EMU_REGISTERS_REGISTERS],
        m_waveform: unsafe { MaybeUninit::zeroed().assume_init() },
    });

    let fm_operator = OplEmuFmOperator {
        m_choffs: 0,
        m_opoffs: 0,
        m_phase: 0,
        m_env_attenuation: 0,
    ...
    println!("Channel Offset: {}", fm_operator.opl_emu_fm_operator_choffs());
}
