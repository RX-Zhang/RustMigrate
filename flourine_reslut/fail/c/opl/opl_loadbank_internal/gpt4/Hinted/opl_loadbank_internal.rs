...
fn opl_loadbank_internal(opl: &mut Opl, file: &str, offset: i32) -> io::Result<i32> {
    ...
    for i in offset as usize..128 + offset as usize {
        f.read_exact(&mut buff)?;
        let modulator_E862 = u32::from(buff[8]) << 24 | u32::from(buff[6]) << 16 | u32::from(buff[4]) << 8 | u32::from(buff[0]);
        let carrier_E862 = u32::from(buff[9]) << 24 | u32::from(buff[7]) << 16 | u32::from(buff[5]) << 8 | u32::from(buff[1]);
