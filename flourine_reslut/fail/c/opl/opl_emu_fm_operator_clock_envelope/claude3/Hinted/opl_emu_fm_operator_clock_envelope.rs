...
fn opl_emu_fm_operator_clock_envelope(fmop: &mut OplEmuFmOperator, env_counter: u32) {
    ...
    if fmop.m_env_state == OplEmuEnvelopeState::OplEmuEgDecay
        && u32::from(fmop.m_env_attenuation) >= u32::from(fmop.m_cache.eg_sustain)
    {
    ...
    let mut env_counter = Wrapping(env_counter);
    for _ in 0..rate_shift {
        env_counter = Wrapping(env_counter.0.wrapping_shl(1));
    }

    if opl_emu_bitfield(env_counter.0, 0, 11) != 0 {
        return;
    }
    ...
    if fmop.m_env_state == OplEmuEnvelopeState::OplEmuEgAttack {
        if rate < 62 {
            fmop.m_env_attenuation = fmop
                .m_env_attenuation
                .wrapping_add(((!fmop.m_env_attenuation).wrapping_mul(increment.try_into().unwrap()) >> 4) as u16);
        }
    } else {
        fmop.m_env_attenuation = fmop
            .m_env_attenuation
            .wrapping_add((increment as u16).wrapping_mul(16));
