...
fn opl_emu_fm_operator_clock_envelope(fmop: &mut OplEmuFmOperator, env_counter: u32) {
    ...
    let rate = fmop.m_cache.eg_rate[fmop.m_env_state as usize] as u32;
    ...
    if fmop.m_env_state == OplEmuEnvelopeState::Attack {
        if rate < 62 {
            fmop.m_env_attenuation = ((0x3FFu32 - fmop.m_env_attenuation as u32) * increment as u32 >> 4) as u16 + fmop.m_env_attenuation;
        }
    } else {
        fmop.m_env_attenuation = (fmop.m_env_attenuation as u32 + increment).min(0x3FF) as u16;
    }
}
