...
fn opl_emu_fm_operator_clock_envelope(fmop: &mut OplEmuFmOperator, env_counter: u32) {
    ...
    if fmop.m_env_state == OplEmuEnvelopeState::Attack {
        if rate < 62 {
            fmop.m_env_attenuation = ((0x3FF as u32 - u32::from(fmop.m_env_attenuation)) * increment as u32 >> 4) as u16 + fmop.m_env_attenuation;
        }
    } else {
        fmop.m_env_attenuation = (u32::from(fmop.m_env_attenuation) + increment).min(0x3FF) as u16;
    }
}
