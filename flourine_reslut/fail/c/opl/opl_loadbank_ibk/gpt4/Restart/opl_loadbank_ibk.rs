...
fn opl_loadbank_internal(opl: &mut Opl, file: &str, offset: usize) -> io::Result<i32> {
    ...
    let signature = &buff[0..4];
    if signature != [b'I', b'B', b'K', 0x1A] {
        return Ok(-3);
    }
...
fn opl_loadbank_ibk(opl: &mut Opl, file: &str) -> io::Result<i32> {
    let mut instruments = file.to_owned();
    let mut percussion = None;
    if let Some(idx) = instruments.find(',') {
        percussion = Some(instruments.split_off(idx + 1));
        instruments.truncate(idx);
    }
