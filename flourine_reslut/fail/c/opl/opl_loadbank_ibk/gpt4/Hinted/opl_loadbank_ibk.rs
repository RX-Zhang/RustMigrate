...
fn opl_loadbank_internal(opl: &mut Opl, file: &str, offset: usize) -> io::Result<i32> {
    opl.is_op2 = 0;
    let mut buff = [0u8; 16];
    ...
    for i in offset..(128 + offset) {
        ...
        timbre.modulator_E862 = u32::from(buff[8]) << 24 | u32::from(buff[6]) << 16 | u32::from(buff[4]) << 8 | u32::from(buff[0]);
        timbre.carrier_E862 = u32::from(buff[9]) << 24 | u32::from(buff[7]) << 16 | u32::from(buff[5]) << 8 | u32::from(buff[1]);
...
fn opl_loadbank_ibk(opl: &mut Opl, file: &str) -> io::Result<i32> {
    ...
    if res == 0 {
        if let Some(percussion) = percussion {
            return opl_loadbank_internal(opl, percussion, 128);
        }
    }
    Ok(res)
}
