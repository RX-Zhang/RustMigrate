...
fn opl_loadbank_internal(opl: &mut Opl, file: &Path, offset: usize) -> io::Result<i32> {
    opl.is_op2 = false;
    let mut buff = [0u8; 16];
    ...
    if &buff[0..4] != [b'I', b'B', b'K', 0x1A] {
        return Ok(-3);
    }
    for i in offset..(128 + offset) {
        f.read_exact(&mut buff)?;
        let timbre = &mut opl.opl_gmtimbres[i];
        timbre.modulator_E862 = (buff[8] as u32) << 24 | (buff[6] as u32) << 16 | (buff[4] as u32) << 8 | buff[0] as u32;
        timbre.carrier_E862 = (buff[9] as u32) << 24 | (buff[7] as u32) << 16 | (buff[5] as u32) << 8 | buff[1] as u32;
...
fn opl_loadbank_ibk(opl: &mut Opl, file: &str) -> io::Result<i32> {
    let (instruments, percussion) = if let Some((inst, perc)) = file.split_once(',') {
        (inst, Some(perc))
    } else {
        (file, None)
    };
    let res = opl_loadbank_internal(opl, Path::new(instruments), 0)?;
    if res == 0 {
        if let Some(percussion) = percussion {
            return opl_loadbank_internal(opl, Path::new(percussion), 128);
        }
    }
    Ok(res)
}
