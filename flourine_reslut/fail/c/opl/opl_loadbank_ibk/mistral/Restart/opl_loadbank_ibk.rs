...
fn opl_loadbank_internal(
    ...
    for i in offset..128 + offset {
        ...
        let notenum = buff[13];
        let noteoffset = (buff[14] as i16) << 8 | buff[15] as i16;
        opl.opl_gmtimbres[i as usize] = OplTimbre {
            modulator_E862,
            carrier_E862,
            modulator_40,
            carrier_40,
...
fn opl_loadbank_ibk(opl: &mut Opl, file: &str) -> io::Result<()> {
    ...
    if index.is_some() {
        percussion = instruments.drain(index.unwrap() + 1..).collect();
        instruments.truncate(index.unwrap());
    }
    opl_loadbank_internal(opl, instruments.as_str(), 0)?;
    if !percussion.is_empty() {
        opl_loadbank_internal(opl, percussion.as_str(), 128)?;
    }
    Ok(())
}
