...
fn opl_loadbank_ibk(opl: &mut OplT, file: &str) -> i32 {
    let instruments = match std::fs::read(file) {
        Ok(bytes) => bytes.into_boxed_slice(),
        Err(_) => return -64,
    };

    let mut percussion: Option<Box<Vec<u8>>> = None;
    let mut i = 0;

    while i < instruments.len() {
        if instruments[i] == b',' {
            percussion = Some(Box::new(instruments.to_vec().split_off(i + 1)));
            break;
        }
        i += 1;
    }

    let res = opl_loadbank_internal(
        opl,
        std::str::from_utf8(&instruments).unwrap(),
        0,
    );
    ...
    if res == 0 && percussion.is_some() {
        opl_loadbank_internal(
            opl,
            std::str::from_utf8(&percussion.unwrap()).unwrap(),
            128,
        )
    } else {
        res
