...
fn opl_emu_opl_clock_noise_and_lfo(
    ...
    *noise_lfsr = noise_lfsr_local;
    ...
    let am_counter = lfo_am_counter.wrapping_add(1);
    ...
    *lfo_am = if am_counter < 105 * 64 {
        ((am_counter >> shift) & 0x7f) as u8
    } else {
        (((210 * 64 + 63) - am_counter) >> shift) as u8 & 0x7f
    };

    // the PM LFO has 8192 steps, or a nominal period of 6.1Hz
    let pm_counter = lfo_pm_counter.wrapping_add(1);
    ...
    (pm_scale[((*lfo_pm_counter >> 10) & 7) as usize] >> (pm_depth ^ 1)) as i32
}
