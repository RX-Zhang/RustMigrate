...
fn opl_emu_registers_byte(
    ...
    opl_emu_bitfield(regs.m_regdata[(offset + extra_offset) as usize].into(), start as i32, count as i32)
}
...
fn opl_emu_opl_clock_noise_and_lfo(
    ...
    *lfo_pm_counter = lfo_pm_counter.wrapping_add(1);
    ...
    pm_scale[opl_emu_bitfield(pm_counter as u32, 10, 3) as usize] >> (pm_depth ^ 1)
}
