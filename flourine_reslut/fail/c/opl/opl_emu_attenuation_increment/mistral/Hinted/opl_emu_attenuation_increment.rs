...
fn opl_emu_bitfield(value: u32, start: i32, length: i32) -> u32 {
    assert!(start >= 0 && length >= 0 && start + length <= 32, "Input is invalid");
...
fn opl_emu_attenuation_increment(rate: u32, index: u32) -> u32 {
    assert!(
        rate <= 63 && index <= 15,
        "Input is invalid, rate or index is out of bounds"
    );
    let shifted_index = (index as i32) * 4;
    let shifted_rate = shifted_index + 4;
    let table_value = INCREMENT_TABLE[rate as usize];
    opl_emu_bitfield(table_value, shifted_rate, 4)
}
