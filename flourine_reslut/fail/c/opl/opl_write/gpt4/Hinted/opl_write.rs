...
struct OplEmuOpdataCache {
    ...
    multiple: u32,
    eg_sustain: u32,
    eg_rate: [u8; OplEmuEnvelopeState::OplEmuEgRelease as usize + 1],
    eg_shift: u8,
}
...
fn opl_emu_fm_operator_keyonoff(fmop: &mut OplEmuFmOperator, on: u32, type_: OplEmuKeyonType) {
    fmop.m_keyon_live = ((fmop.m_keyon_live as u32) & !(1 << type_ as u32)) as u8 | (opl_emu_bitfield(on, 0, 1) << type_ as u32) as u8;
}
    ...
    for i in 0..count as usize {
        ...
        if opl_emu_registers_write(&mut emu.m_regs, regnum, value, &mut keyon_channel, &mut keyon_opmask) {
            if keyon_channel < OPL_EMU_REGISTERS_CHANNELS as u32 {
                ...
                opl_emu_fm_channel_keyonoff(&mut emu.m_channel[6], opl_emu_bitfield(keyon_opmask, 4, 1) * 3, OplEmuKeyonType::OplEmuKeyonRhythm, 6);
                opl_emu_fm_channel_keyonoff(&mut emu.m_channel[7], (opl_emu_bitfield(keyon_opmask, 0, 1) as u32) | ((opl_emu_bitfield(keyon_opmask, 3, 1) as u32) << 1), OplEmuKeyonType::OplEmuKeyonRhythm, 7);
                opl_emu_fm_channel_keyonoff(&mut emu.m_channel[8], (opl_emu_bitfield(keyon_opmask, 2, 1) as u32) | ((opl_emu_bitfield(keyon_opmask, 1, 1) as u32) << 1), OplEmuKeyonType::OplEmuKeyonRhythm, 8);
            }
        }
    }
}
