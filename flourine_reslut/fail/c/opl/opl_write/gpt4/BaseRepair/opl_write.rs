    ...
    for i in 0..count as usize {
        ...
        if opl_emu_registers_write(&mut emu.m_regs, regnum, value, &mut keyon_channel, &mut keyon_opmask) {
            if keyon_channel < OPL_EMU_REGISTERS_CHANNELS as u32 {
                ...
                opl_emu_fm_channel_keyonoff(&mut emu.m_channel[6], opl_emu_bitfield(keyon_opmask, 4, 1) as u32, OplEmuKeyonType::Rhythm, 6);
                opl_emu_fm_channel_keyonoff(&mut emu.m_channel[7], (opl_emu_bitfield(keyon_opmask, 0, 1) as u32 | (opl_emu_bitfield(keyon_opmask, 3, 1) as u32) << 1), OplEmuKeyonType::Rhythm, 7);
                opl_emu_fm_channel_keyonoff(&mut emu.m_channel[8], (opl_emu_bitfield(keyon_opmask, 2, 1) as u32 | (opl_emu_bitfield(keyon_opmask, 1, 1) as u32) << 1), OplEmuKeyonType::Rhythm, 8);
            }
        }
    }
}
