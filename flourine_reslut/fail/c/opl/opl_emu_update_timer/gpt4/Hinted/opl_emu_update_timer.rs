...
impl OplEmu {
    ...
    fn opl_emu_registers_timer_b_value(regs: &OplEmuRegisters) -> u32 {
        Self::opl_emu_registers_byte(regs, 0x03, 0, 8, 0)
    }

    fn opl_emu_registers_timer_a_value(regs: &OplEmuRegisters) -> u32 {
        Self::opl_emu_registers_byte(regs, 0x02, 0, 8, 0) * 4
    }

    fn opl_emu_update_timer(&mut self, tnum: u32, enable: u32) {
        if enable != 0 && self.m_timer_running[tnum as usize] == 0 {
            let period = if tnum == 0 {
                1024 - Self::opl_emu_registers_timer_a_value(&self.m_regs)
            } else {
                16 * (256 - Self::opl_emu_registers_timer_b_value(&self.m_regs))
            };
            self.m_timer_running[tnum as usize] = 1;
        } else if enable == 0 {
            self.m_timer_running[tnum as usize] = 0;
        }
    }
}
