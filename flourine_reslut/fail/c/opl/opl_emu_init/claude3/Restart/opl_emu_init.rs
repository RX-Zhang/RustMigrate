...
fn opl_emu_fm_channel_assign(
    ...
    let mut op_ref = op;
    fmch.m_op[index as usize] = op_ref.take();
    if let Some(op) = op_ref.as_mut() {
    ...
    for chnum in 0..OPL_EMU_REGISTERS_CHANNELS {
        for index in 0..4 {
            ...
            let op = if opnum == 0xff {
                ...
                Some(Box::new(OplEmuFmOperator {
                    m_choffs: 0,
                    m_opoffs: opl_emu_registers_operator_offset(opnum),
                    m_phase: 0,
...
fn opl_emu_registers_init(regs: &mut OplEmuRegisters) {
    ...
    if OPL_EMU_REGISTERS_WAVEFORMS >= 4 {
        ...
        for index in 0..OPL_EMU_REGISTERS_WAVEFORM_LENGTH {
            regs.m_waveform[1][index] =
                if opl_emu_bitfield(index as u32, 9, 1) != 0 {
                    zeroval
                } else {
                    regs.m_waveform[0][index]
                };
            regs.m_waveform[2][index] = regs.m_waveform[0][index] & 0x7fff;
            regs.m_waveform[3][index] =
                if opl_emu_bitfield(index as u32, 8, 1) != 0 {
                    zeroval
                } else {
                    regs.m_waveform[0][index] & 0x7fff
                };
            if OPL_EMU_REGISTERS_WAVEFORMS >= 8 {
                regs.m_waveform[4][index] =
                    if opl_emu_bitfield(index as u32, 9, 1) != 0 {
                        zeroval
                    } else {
                        regs.m_waveform[0][index.wrapping_mul(2)]
                    };
                regs.m_waveform[5][index] =
                    if opl_emu_bitfield(index as u32, 9, 1) != 0 {
                        zeroval
                    } else {
                        regs.m_waveform[0][(index.wrapping_mul(2)) & 0x1ff]
                    };
                regs.m_waveform[6][index] = (opl_emu_bitfield(index as u32, 9, 1) << 15) as u16;
                regs.m_waveform[7][index] =
                    ((if opl_emu_bitfield(index as u32, 9, 1) != 0 {
                        (index ^ 0x13ff).wrapping_add(index)
                    } else {
                        index
                    }) << 3) as u16;
            }
