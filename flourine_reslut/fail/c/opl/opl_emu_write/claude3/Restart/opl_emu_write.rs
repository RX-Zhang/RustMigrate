...
struct OplEmuOpdataCache {
    ...
    multiple: u32,
    eg_sustain: u32,
    eg_rate: [u8; OPL_EMU_REGISTERS_OPERATORS],
    eg_shift: u8,
}
...
fn opl_emu_fm_operator_keyonoff(fmop: &mut OplEmuFmOperator, on: u32, type_: u8) {
    fmop.m_keyon_live = (fmop.m_keyon_live & !(1 << type_))
        | ((opl_emu_bitfield(on, 0, 1) << type_) as u8);
}
    ...
    if opl_emu_registers_write(
        ...
        if keyon_channel < OPL_EMU_REGISTERS_CHANNELS as u32 {
            opl_emu_fm_channel_keyonoff(
                ...
                keyon_opmask,
                OplEmuKeyonType::OplEmuKeyonNormal as u8,
                keyon_channel,
            );
        } else if OPL_EMU_REGISTERS_CHANNELS >= 9 && keyon_channel == OPL_EMU_REGISTERS_RHYTHM_CHANNEL
        {
            opl_emu_fm_channel_keyonoff(
                ...
                OplEmuKeyonType::OplEmuKeyonRhythm as u8,
                6,
            );
            opl_emu_fm_channel_keyonoff(
                &mut emu.m_channel[7],
