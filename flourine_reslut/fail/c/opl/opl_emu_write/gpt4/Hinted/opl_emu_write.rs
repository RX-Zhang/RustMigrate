...
fn opl_emu_fm_operator_keyonoff(fmop: &mut OplEmuFmOperator, on: u32, type_: OplEmuKeyonType) {
    fmop.m_keyon_live = ((fmop.m_keyon_live as u32 & !(1 << (type_ as i32))) | (opl_emu_bitfield(on, 0, 1) << (type_ as i32))) as u8;
}

fn opl_emu_fm_channel_keyonoff(fmch: &mut OplEmuFmChannel, states: u32, type_: OplEmuKeyonType, chnum: u32) {
    for op in fmch.m_op.iter_mut().filter_map(|op| op.as_mut()) {
...
fn opl_emu_write(emu: &mut OplEmu, regnum: u16, data: u8) {
    ...
    if opl_emu_registers_write(&mut emu.m_regs, regnum, data, &mut keyon_channel, &mut keyon_opmask) {
        if keyon_channel < OPL_EMU_REGISTERS_CHANNELS as u32 {
            ...
            opl_emu_fm_channel_keyonoff(&mut emu.m_channel[6], opl_emu_bitfield(keyon_opmask, 4, 1) as u32 * 3, OplEmuKeyonType::Rhythm, 6);
            opl_emu_fm_channel_keyonoff(&mut emu.m_channel[7], (opl_emu_bitfield(keyon_opmask, 0, 1) | (opl_emu_bitfield(keyon_opmask, 3, 1) << 1)) as u32, OplEmuKeyonType::Rhythm, 7);
            opl_emu_fm_channel_keyonoff(&mut emu.m_channel[8], (opl_emu_bitfield(keyon_opmask, 2, 1) | (opl_emu_bitfield(keyon_opmask, 1, 1) << 1)) as u32, OplEmuKeyonType::Rhythm, 8);
        }
    }
}
