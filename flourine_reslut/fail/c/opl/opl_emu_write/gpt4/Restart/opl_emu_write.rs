...
fn opl_emu_fm_channel_keyonoff(fmch: &mut OplEmuFmChannel, states: u32, type_: OplEmuKeyonType, chnum: u32) {
    for op in fmch.m_op.iter_mut().filter_map(|op| op.as_mut()) {
        opl_emu_fm_operator_keyonoff(op, opl_emu_bitfield(states, op.m_choffs as i32, 1), type_);
    }
}
...
fn opl_emu_write(emu: &mut OplEmuT, regnum: u16, data: u8) {
    ...
    if opl_emu_registers_write(&mut emu.m_regs, regnum, data, &mut keyon_channel, &mut keyon_opmask) != 0 {
        if keyon_channel < OPL_EMU_REGISTERS_CHANNELS as u32 {
            ...
            opl_emu_fm_channel_keyonoff(&mut emu.m_channel[6], (opl_emu_bitfield(keyon_opmask, 4, 1) != 0) as u32 * 3, OplEmuKeyonType::OplEmuKeyonRhythm, 6);
