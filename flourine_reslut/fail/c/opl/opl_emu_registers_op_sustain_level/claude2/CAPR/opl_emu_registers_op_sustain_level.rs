...
fn opl_emu_registers_byte(
    ...
    let regdata: &u8 = &regs.m_regdata[<usize as TryInto<usize>>::try_into((offset.wrapping_add(extra_offset)).try_into().unwrap()).unwrap()];
    let regdata_value = *regdata as u32;
    
    opl_emu_bitfield(regdata_value, start.try_into().unwrap(), count.try_into().unwrap())
}
