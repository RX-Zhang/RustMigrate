...
fn opl_emu_fm_operator_envelope_attenuation(fmop: &OplEmuFmOperator, am_offset: u16) -> u32 {
    ...
    if opl_emu_registers_op_lfo_am_enable(&fmop.m_regs, fmop.m_opoffs) != 0 {
        result = result.wrapping_add(am_offset as u32);
    }

    result = result.wrapping_add(fmop.m_cache.total_level);
    opl_min(result, 0x3ff_u32)
}
