...
fn registers_op_lfo_am_enable(regs: &Box<Registers>, opoffs: u32) -> u32 {
    registers_byte(regs, 0x20, 7, 1, opoffs)
}

fn fm_operator_envelope_attenuation(fmop: &FmOperator, am_offset: u32) -> u32 {
    let mut result = fmop.env_attenuation as u32 >> fmop.cache.eg_shift as u32;

    if registers_op_lfo_am_enable(&fmop.regs, fmop.opoffs) != 0 {
        result = result.wrapping_add(am_offset);
    }

    result.wrapping_add(fmop.cache.total_level).min(0x3ff) as u32
}
