...
fn opl_loadbank_op2(opl: &mut Opl, data: &[u8]) -> Result<(), i32> {
    ...
    for i in 0..175 {
        ...
        let op2_flags = u16::from(buff[0]) | (u16::from(buff[1]) << 8);
        let finetune = buff[2];
        ...
        opl.opl_gmtimbres_voice2[i].finetune = (opl.opl_gmtimbres_voice2[i].finetune as i8).wrapping_add((finetune - 128) as i8) as i8;
