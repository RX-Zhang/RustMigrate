...
fn aptx_reset(ctx: &mut AptxContext) {
    ...
    ctx.hd = hd;
    ctx.decode_skip_leading = ((LATENCY_SAMPLES as usize + 3) / (4 as usize)) as u8;
    ctx.encode_remaining = ((LATENCY_SAMPLES as usize + 3) / (4 as usize)) as u8;
...
fn aptx_init(hd: i32) -> Box<AptxContext> {
    let mut ctx: Box<AptxContext> = Box::new(AptxContext{
        ...
        decode_dropped: 0, 
        channels: unsafe { std::mem::MaybeUninit::uninit().assume_init() },
        hd: 0,
        sync_idx: 0,
        encode_remaining: 0,
