...
fn aptx_unpack_codeword(channel: &mut AptxChannel, codeword: u16) {
    channel.quantize[0].quantized_sample = sign_extend((codeword >> 0).into(), 7);
    channel.quantize[1].quantized_sample = sign_extend((codeword >> 7).into(), 4);
    channel.quantize[2].quantized_sample = sign_extend((codeword >> 11).into(), 2);
    channel.quantize[3].quantized_sample = sign_extend((codeword >> 13).into(), 3);
