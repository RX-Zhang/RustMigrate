...
impl AptxPrediction {
    ...
    fn aptx_reconstructed_differences_update(&mut self, reconstructed_difference: i32, order: usize) {
        let p = self.pos;
    ...
    fn aptx_prediction_filtering(&mut self, reconstructed_difference: i32, order: usize) {
        let reconstructed_sample = Self::clip_intp2(reconstructed_difference + self.predicted_sample, 23);
        let predictor = Self::clip_intp2(((self.s_weight[0] as i64 * self.previous_reconstructed_sample as i64
            + self.s_weight[1] as i64 * reconstructed_sample as i64) >> 22) as i32, 23);
        self.previous_reconstructed_sample = reconstructed_sample;

        self.aptx_reconstructed_differences_update(reconstructed_difference, order);
