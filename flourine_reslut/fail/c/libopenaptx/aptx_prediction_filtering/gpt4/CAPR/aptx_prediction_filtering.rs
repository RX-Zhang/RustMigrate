...
impl AptxPrediction {
    ...
    fn aptx_reconstructed_differences_update(&mut self, reconstructed_difference: i32, order: usize) -> &mut i32 {
        ...
        self.reconstructed_differences[p] = self.reconstructed_differences[p % order];
        self.pos = (p + 1) % order;
        self.reconstructed_differences[p % order] = reconstructed_difference;
        &mut self.reconstructed_differences[p % order]
    }
    ...
    fn aptx_prediction_filtering(&mut self, reconstructed_difference: i32, order: usize) {
        ...
        let predictor = Self::clip_intp2(((self.s_weight[0] as i64 * self.previous_reconstructed_sample as i64 + self.s_weight[1] as i64 * reconstructed_sample as i64) >> 22) as i32, 23);
        self.previous_reconstructed_sample = reconstructed_sample;

        let mut predicted_difference: i64 = 0;
        for i in 0..order {
            let srd = (self.reconstructed_differences[(self.pos + order - i - 1) % order].wrapping_shr(31) | 1) as i32;
            let srd0 = (reconstructed_difference.signum() * ((1 << 23) as i32)) as i32;
            self.d_weight[i] -= Self::rshift32(self.d_weight[i] - srd * srd0, 8);
            predicted_difference += self.reconstructed_differences[(self.pos + order - i) % order] as i64 * self.d_weight[i] as i64;
        }

        self.predicted_difference = Self::clip_intp2((predicted_difference >> 22) as i32, 23);
        self.predicted_sample = Self::clip_intp2(predictor.wrapping_add(self.predicted_difference), 23);
    }
}
