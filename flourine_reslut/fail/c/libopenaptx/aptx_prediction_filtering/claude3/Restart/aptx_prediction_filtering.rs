...
fn aptx_reconstructed_differences_update(
    ...
    &mut prediction.reconstructed_differences[(order + p) % prediction.reconstructed_differences.len()]
}
...
fn aptx_prediction_filtering(
    ...
    let srd0 = (reconstructed_difference > 0) as i32 * (1 << 23)
        - (reconstructed_difference < 0) as i32 * (1 << 23);
    let mut predicted_difference = 0;
    for i in 0..order {
        let srd = (prediction.reconstructed_differences[(prediction.reconstructed_differences.len() - i - 1)] >> 31) | 1;
