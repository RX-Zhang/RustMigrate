...
fn aptx_insert_sync(channels: &mut [AptxChannel; 2], sync_idx: &mut u8) {
    ...
    for (channel_idx, c) in channels.iter().enumerate() {
        for (i, q) in c.quantize.iter().enumerate() {
            if q.error < min_error {
                min_error = q.error;
                min_idx = i;
    ...
    if aptx_check_parity(channels, sync_idx) != 0 {
        channels[min_channel_idx].quantize[map[min_idx]].quantized_sample =
            channels[min_channel_idx].quantize[map[min_idx]].quantized_sample_parity_change;
    }
}
