...
fn aptx_invert_quantization(
    ...
    idx = if quantized_sample < 0 { -(quantized_sample + 1) } else { quantized_sample + 1 };
    qr = *tables.quantize_intervals.as_ref().get(idx as usize).unwrap() / 2;
    if quantized_sample < 0 {
        qr = -qr;
    }

    qr = rshift64_clip24(
        (qr as i64 * (1i64<<32)) + (dither as i64 * *tables.invert_quantize_dither_factors.as_ref().get(idx as usize).unwrap() as i64),
        32
    ) as i32;
    ...
    factor_select = rshift32(factor_select.wrapping_add((*tables.quantize_factor_select_offset.as_ref().get(idx as usize).unwrap()) as i32 * (1 << 15)), 15);
