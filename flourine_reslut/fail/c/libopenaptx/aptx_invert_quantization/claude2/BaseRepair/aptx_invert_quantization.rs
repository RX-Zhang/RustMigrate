...
fn rshift64(value: i64, shift: u32) -> i64 {
    ...
    ((value + rounding as i64) >> shift) - (((value & mask) == rounding as i64) as i64)
}
...
fn aptx_invert_quantization(
    ...
    let mut qr = 0;
    let negative = quantized_sample < 0;
    let idx = (!negative as i32 ^ quantized_sample).wrapping_add(1);
    ...
    qr = rshift64_clip24(
        (qr as i64 * (1 << 32))
            .wrapping_add((dither * *tables.invert_quantize_dither_factors.get(idx as usize).unwrap()) as i64),
        32,
    );
    invert_quantize.reconstructed_difference = ((invert_quantize.quantization_factor as i64 * qr as i64) >> 19) as i32;

    let mut factor_select = (32620 * invert_quantize.factor_select) as i32;
    factor_select = rshift32(
        factor_select.wrapping_add((*tables.quantize_factor_select_offset.get(idx as usize).unwrap() as i32 * (1 << 15)) as i32),
        15,
    );
    invert_quantize.factor_select = clip(factor_select, 0, tables.factor_max);

    let idx = (invert_quantize.factor_select & 0xFF) >> 3;
    let shift: i32 = (tables.factor_max - invert_quantize.factor_select) >> 8;
    invert_quantize.quantization_factor = ((*QUANTIZATION_FACTORS.get(idx as usize).unwrap() as i32) << 11) >> shift;
}
