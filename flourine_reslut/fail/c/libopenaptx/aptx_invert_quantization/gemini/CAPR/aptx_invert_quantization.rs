...
pub struct AptxTables {
    pub quantize_intervals: Box<[i32]>,
    pub invert_quantize_dither_factors: Box<[i32]>,
    pub quantize_dither_factors: Box<[i32]>,
    pub quantize_factor_select_offset: Box<[i16]>,
...
pub fn aptx_invert_quantization(
    ...
    idx = (quantized_sample.abs() as usize) + 1;
    ...
    factor_select = rshift32(
        factor_select + (tables.quantize_factor_select_offset[idx] * (1 << 15)) as i32,
        15,
    );
