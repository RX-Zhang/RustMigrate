...
fn rshift64_clip24(value: i64, shift: u32) -> i32 {
    return clip_intp2(value as i32, 23);
}

fn rshift32(value: i32, shift: u32) -> i32 {
    let rounding = 1 << (shift - 1);
    let mask = (1 << (shift + 1)) - 1;
    return ((value.wrapping_add(rounding)) >> shift).wrapping_sub(((value & mask) == rounding) as i32);
}
...
fn aptx_invert_quantization(
    ...
    let idx = (quantized_sample as u32).wrapping_add(!(quantized_sample < 0) as u32) + 1;
    let qr = tables.quantize_intervals[idx as usize] / 2;
    ...
    factor_select = rshift32(
        (factor_select as i64)
            .wrapping_add(
                (tables.quantize_factor_select_offset[idx as usize]
                    * (1 << 15)) as i64,
            ) as i32,
        15,
    );
