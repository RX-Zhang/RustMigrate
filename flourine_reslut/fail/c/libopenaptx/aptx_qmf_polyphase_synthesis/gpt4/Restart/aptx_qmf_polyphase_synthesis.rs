...
fn main() {
    // Example usage
    let mut signal = [AptxFilterSignal::new(), AptxFilterSignal::new()];
    let coeffs = [[0; FILTER_TAPS]; NB_FILTERS];
    let mut samples = [0; NB_FILTERS];
    aptx_qmf_polyphase_synthesis(&mut signal, &coeffs, 0, 0, 0, &mut samples);
}
