...
fn aptx_pack_codeword(channel: &AptxChannel) -> u16 {
    ...
    (((channel.quantize[3].quantized_sample & 0x06) | parity) << 13)
        .wrapping_add((channel.quantize[2].quantized_sample & 0x03) << 11)
        .wrapping_add((channel.quantize[1].quantized_sample & 0x0F) << 7)
        .wrapping_add((channel.quantize[0].quantized_sample & 0x7F) << 0) as u16
}
