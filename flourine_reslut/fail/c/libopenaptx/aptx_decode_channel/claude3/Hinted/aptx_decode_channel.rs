...
fn aptx_qmf_polyphase_synthesis(
    ...
    samples: &mut [Box<[i32; NB_FILTERS]>; 2],
) {
...
fn aptx_qmf_tree_synthesis(
    ...
    let mut intermediate_samples: [Box<[i32; NB_FILTERS]>; 2] = unsafe { mem::uninitialized() };
    ...
    for i in 0..2 {
        aptx_qmf_polyphase_synthesis(
            ...
            21,
            intermediate_samples[i][0],
            intermediate_samples[i][1],
            &mut [
                Box::new([samples[2 * i], samples[2 * i + 1]]),
                Box::new([0, 0]),
            ],
        );
    }
