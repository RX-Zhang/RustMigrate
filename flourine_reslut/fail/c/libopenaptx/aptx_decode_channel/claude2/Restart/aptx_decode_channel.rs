...
fn clip_intp2(a: i32, p: u32) -> i32 {
    if ((a as u32).wrapping_add((1 << p) as u32)) & !(((1 << (p + 1)) - 1) as u32) != 0 {
        ((a >> 31) ^ ((1 << p) - 1)) as i32
    } else {
        a
    }
...
fn aptx_qmf_convolution(signal: &AptxFilterSignal, coeffs: &[i32; FILTER_TAPS], shift: u32) -> i32 {
    ...
    for i in 0..FILTER_TAPS {
        e += (signal.buffer[signal.pos as usize + i] as i64) * (coeffs[i] as i64);
    }
...
fn aptx_qmf_filter_signal_push(signal: &mut AptxFilterSignal, sample: i32) {
    ...
    signal.buffer[(signal.pos as usize + FILTER_TAPS) % FILTER_TAPS] = sample;
    signal.pos = signal.pos.wrapping_add(1);
}

fn aptx_qmf_polyphase_synthesis(
    signals: &mut [AptxFilterSignal; NB_FILTERS],
...
fn aptx_qmf_tree_synthesis(
    ...
    for i in 0..2 {
        ...
        samples[2*i] = output[0];
        samples[2*i+1] = output[1];
    }
}
