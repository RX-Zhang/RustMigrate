...
fn aptx_quantize_difference(quantize: &mut AptxQuantize, sample_difference: i32, dither: i32,
    ...
    let mut inv: i32;
    let mut sample_difference_abs: i32;
    let mut error: i64;
    ...
    let mut intervals = &mut intervals[quantized_sample as usize..];
    mean = (intervals[1] + intervals[0]) / 2;
    interval = (intervals[1] - intervals[0]) * (1 - ((sample_difference < 0) as i32 * 2));

    dithered_sample = rshift64_clip24((dither as i64 * interval as i64 + (clip_intp2(mean + d, 23) as i64) << 32), 32) as i32;
    ...
    quantize.quantized_sample.bitxor_assign(quantized_sample ^ inv);
    quantize.quantized_sample_parity_change.bitxor_assign(parity_change ^ inv);
}
