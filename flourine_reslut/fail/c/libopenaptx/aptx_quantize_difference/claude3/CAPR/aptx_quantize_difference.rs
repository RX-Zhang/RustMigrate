...
fn aptx_quantize_difference(
    ...
    let intervals = &intervals[quantized_sample..];
    let mean = (intervals[1] + intervals[0]) / 2;
    let interval = (intervals[1] - intervals[0]) * (1 - 2 * (sample_difference < 0) as i32);
    ...
    if error < 0 {
        quantize.quantized_sample = quantized_sample as i32 - 1;
    } else {
        parity_change -= 1;
        quantize.quantized_sample = quantized_sample as i32;
    }

    let inv = 1 - 2 * (sample_difference < 0) as i32;
