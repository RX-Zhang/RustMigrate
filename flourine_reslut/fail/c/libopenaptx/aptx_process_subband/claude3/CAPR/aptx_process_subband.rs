...
fn rshift64(value: i64, shift: u32) -> i32 {
    ...
    let masked = (value & mask) as i32;
    shifted - (masked == rounding as i32) as i32
}
...
fn rshift32(value: i32, shift: u32) -> i32 {
    ...
    shifted - (masked == rounding) as i32
}
...
fn aptx_reconstructed_differences_update(
    ...
    rd1[p] = rd1[(order as usize + p) % order as usize];
...
fn aptx_prediction_filtering(prediction: &mut AptxPrediction, reconstructed_difference: i32, order: i32) {
    ...
    let srd0 = (((reconstructed_difference as u32) >> 31) as i32 * 2 - 1) << 23;
    let mut predicted_difference: i32 = 0;
    let mut d_weight = prediction.d_weight.clone();
    for i in 0..order as usize {
        let srd = (prediction.reconstructed_differences[i] >> 31) | 1;
        d_weight[i] = d_weight[i].wrapping_sub(
            rshift32(
        ...
        predicted_difference = predicted_difference.wrapping_add(
            (prediction.reconstructed_differences[i] as i64 * d_weight[i] as i64 >> 22) as i32
        );
    }
    prediction.d_weight = d_weight;
...
fn aptx_invert_quantization(
    ...
    factor_select = rshift32(
        factor_select.wrapping_add(
            (tables.quantize_factor_select_offset[idx as usize] as i32 * (1 << 15)) as i32,
        ),
        15,
    );
...
fn aptx_process_subband(
    ...
    let sign = (invert_quantize.reconstructed_difference
        >= -prediction.predicted_difference) as i32 * 2
        - 1;
    let same_sign = [
