...
fn aptx_reconstructed_differences_update(prediction: &mut AptxPrediction, reconstructed_difference: i32, order: i32) {
...
fn aptx_prediction_filtering(prediction: &mut AptxPrediction, reconstructed_difference: i32, order: i32) {
    ...
    let predictor = clip_intp2(((prediction.s_weight[0] as i64 * prediction.previous_reconstructed_sample as i64 + prediction.s_weight[1] as i64 * reconstructed_sample as i64) >> 22) as i32, 23);
    ...
    for i in 0..order {
        let srd = (prediction.reconstructed_differences[(((prediction.pos as i32 - i - 1) + order) % order) as usize] >> 31) | 1;
        prediction.d_weight[i as usize] -= rshift32(prediction.d_weight[i as usize] - srd * srd0, 8);
        predicted_difference += prediction.reconstructed_differences[(((prediction.pos as i32 - i) + order) % order) as usize] as i64 * prediction.d_weight[i as usize] as i64;
    }
...
fn aptx_invert_quantization(invert_quantize: &mut AptxInvertQuantize, quantized_sample: i32, dither: i32, tables: &AptxTables) {
    let idx = (quantized_sample.abs() ^ (quantized_sample < 0) as i32) + 1;
    let mut qr = tables.quantize_intervals[idx as usize] / 2;
    if quantized_sample < 0 {
        qr = -qr;
    }

    qr = rshift64_clip24(((qr as i64) << 32) + dither as i64 * tables.invert_quantize_dither_factors[idx as usize] as i64, 32);
