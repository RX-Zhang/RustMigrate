...
pub fn opl_emu_registers_byte(regs: &OplEmuRegisters, offset: u32, start: u32, count: u32, extra_offset: u32) -> u32 {
    let total_offset = (offset.wrapping_add(extra_offset) % OPL_EMU_REGISTERS_REGISTERS as u32) as usize;
    opl_emu_bitfield(regs.m_regdata[total_offset] as u32, start as i32, count as i32)
}
