...
// Function to handle key on/off for an FM operator
fn opl_emu_fm_operator_keyonoff(fmop: &mut OplEmuFmOperator, on: u32, type_: OplEmuKeyonType) {
    let type_value = type_ as i32;
    let bit = opl_emu_bitfield(on, 0, 1) << type_value;
    fmop.m_keyon_live = (fmop.m_keyon_live & !(1 << type_value)) | bit as u8;
}
