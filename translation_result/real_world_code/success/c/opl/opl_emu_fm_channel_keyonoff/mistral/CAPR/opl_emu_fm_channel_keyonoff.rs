...
fn opl_emu_fm_operator_keyonoff(fmop: &mut OplEmuFmOperator, on: bool, ty: OplEmuKeyonType) {
    fmop.m_keyon_live = (fmop.m_keyon_live & !(1 << (ty as usize))) | ((on as u8) << (ty as usize));
}

fn opl_emu_fm_channel_keyonoff(fmch: &mut OplEmuFmChannel, states: u32, ty: OplEmuKeyonType, chnum: usize) {
    for (opnum, op) in fmch.m_op.iter_mut().enumerate() {
        if op.is_some() {
            let fmop = op.as_mut().unwrap();
            opl_emu_fm_operator_keyonoff(fmop, states & (1 << opnum) != 0, ty);
        }
    }
}
