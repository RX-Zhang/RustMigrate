...
fn opl_emu_fm_operator_keyonoff(fmop: &mut OplEmuFmOperator, on: u32, typ: OplEmuKeyonType) {
    fmop.m_keyon_live = (fmop.m_keyon_live & !(1 << (typ as u8))).wrapping_add(
        (opl_emu_bitfield(on, 0, 1) << (typ as u8)) as u8
    );
}
...
fn opl_emu_fm_channel_keyonoff(fmch: &mut OplEmuFmChannel, states: u32, typ: OplEmuKeyonType, chnum: u32) {
    for (opnum, op) in fmch.m_op.iter_mut().enumerate() {
        if let Some(op) = op {
            opl_emu_fm_operator_keyonoff(op, opl_emu_bitfield(states, opnum as i32, 1), typ);
        }
    }
}
