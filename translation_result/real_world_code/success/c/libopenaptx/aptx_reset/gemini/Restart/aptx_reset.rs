...
struct AptxContext {
    ...
    channels: [AptxChannel; Channels::NB_CHANNELS],
    hd: u8,
    sync_idx: u8,
    encode_remaining: u8,
...
pub fn aptx_reset(ctx: &mut AptxContext) {
    let hd = ctx.hd;
    let all_zeros: &[u8; std::mem::size_of::<AptxContext>()] =
        &[0; std::mem::size_of::<AptxContext>()];
    unsafe {
        std::ptr::copy_nonoverlapping(all_zeros.as_ptr(), ctx as *mut _ as *mut u8, all_zeros.len());
    }
    ctx.hd = hd;
    ctx.decode_skip_leading = ((LATENCY_SAMPLES + 3) / 4) as u8;
    ctx.encode_remaining = ((LATENCY_SAMPLES + 3) / 4) as u8;
    for chan in 0..Channels::NB_CHANNELS {
