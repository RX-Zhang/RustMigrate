...
fn aptx_reset(ctx: &mut AptxContext) {
    ...
    for i in 0..std::mem::size_of::<AptxContext>() {
        unsafe {
            let ctx_ptr = (ctx as *mut AptxContext).cast::<u8>().add(i);
            *ctx_ptr = 0;
        }
    }

    ctx.hd = hd;
    ctx.decode_skip_leading = ((LATENCY_SAMPLES as u32 + 3) / 4) as u8;
    ctx.encode_remaining = ((LATENCY_SAMPLES as u32 + 3) / 4) as u8;
