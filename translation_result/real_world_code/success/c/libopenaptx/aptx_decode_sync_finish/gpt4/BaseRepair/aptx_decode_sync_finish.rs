...
fn aptx_reset(ctx: &mut AptxContext) {
    ...
    for i in 0..std::mem::size_of::<AptxContext>() {
        // Create a mutable byte-slice view of ctx to zero out all bytes
        let ctx_bytes = unsafe {
            std::slice::from_raw_parts_mut((ctx as *mut AptxContext) as *mut u8,
    ...
    ctx.hd = hd;
    ctx.decode_skip_leading = ((LATENCY_SAMPLES as u8).wrapping_add(3)) / 4;
    ctx.encode_remaining = ((LATENCY_SAMPLES as u8).wrapping_add(3)) / 4;

    for chan in 0..Channel::NbChannels as usize {
