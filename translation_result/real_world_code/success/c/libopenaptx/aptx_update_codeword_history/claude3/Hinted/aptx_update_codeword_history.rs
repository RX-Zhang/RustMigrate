...
fn aptx_update_codeword_history(channel: &mut AptxChannel) {
    ...
    let wrapped_cw = Wrapping(cw as u32);
    let wrapped_history = Wrapping(channel.codeword_history as u32);
    
    channel.codeword_history = ((wrapped_cw << 8) + (wrapped_history << 4)).0 as i32;
}
