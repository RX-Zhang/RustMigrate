...
fn opl_emu_registers_byte(regs: &mut OplEmuRegisters, offset: u32, start: u32, count: u32, extra_offset: u32) -> u32 {
    let real_offset = offset as usize + (extra_offset as usize) % OPL_EMU_REGISTERS_REGISTERS as usize;
    let shifted_value = regs.m_regdata[real_offset] as u32;
    opl_emu_bitfield(shifted_value, start, count)
}

fn opl_emu_registers_op_multiple(regs: &mut OplEmuRegisters, opoffs: u32) -> u32 {
    opl_emu_registers_byte(regs, 0x20, 0, 4, opoffs % OPL_EMU_REGISTERS_REGISTERS as u32)
}
