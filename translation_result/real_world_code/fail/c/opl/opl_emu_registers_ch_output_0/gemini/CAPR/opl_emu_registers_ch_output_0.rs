...
fn opl_emu_registers_byte(regs: &OplEmuRegisters, offset: u32, start: u32, count: u32, extra_offset: u32) -> u32 {
    let combined_offset = (offset as usize).wrapping_add(extra_offset as usize);
    if combined_offset >= OPL_EMU_REGISTERS_REGISTERS {
        0
    } else {
        opl_emu_bitfield(regs.m_regdata[combined_offset] as u32, start as i32, count as i32)
    }
}
