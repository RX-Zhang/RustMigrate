...
fn opl_emu_registers_byte(regs: &mut OplEmuRegisters, offset: u32, start: u32, count: u32, extra_offset: u32) -> u32 {
    ...
    let value = u32::from_le_bytes(regs.m_regdata[real_offset..real_offset + count as usize].try_into().unwrap());
    opl_emu_bitfield(value, start, count)
}

fn opl_emu_registers_op_sustain_level(regs: &mut OplEmuRegisters, opoffs: u32) -> u32 {
