...
fn opl_emu_registers_ch_algorithm(regs: &OplEmuRegisters, choffs: usize) -> u32 {
    let c0 = opl_emu_registers_byte(regs, 0xc0, 0, 1, choffs % OPL_EMU_REGISTERS_REGISTERS);
    let c3 = opl_emu_registers_byte(regs, 0xc3, 0, 1, choffs % OPL_EMU_REGISTERS_REGISTERS);
    c0 | (8 | (c3.wrapping_shl(1)))
}
