...
fn opl_emu_attenuation_to_volume(input: u32) -> u32 {
    let table_index = input & 0xff;
    let shifted_input = (input >> 8) & 0xff;

    let table_entry = POWER_TABLE[table_index as usize];
    let shifted_table_entry = (table_entry as u32).wrapping_shr(shifted_input as u32);
