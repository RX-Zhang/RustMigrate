...
fn opl_emu_registers_byte(regs: &OplEmuRegisters, offset: usize, start: u32, count: u32, extra_offset: usize) -> u32 {
    let index = offset.wrapping_add(extra_offset) as u32;
    let value = regs.m_regdata[index as usize] as u32;
...
fn opl_emu_registers_ch_output_2(regs: &OplEmuRegisters, choffs: usize) -> u32 {
    if opl_emu_registers_newflag(regs) > 0 {
        opl_emu_registers_byte(regs, (0xc0 + choffs) % OPL_EMU_REGISTERS_REGISTERS as usize, 6, 1, 0)
    } else {
        0
    }
}
