...
fn opl_emu_registers_write(regs: &mut OplEmuRegisters, index: usize, data: u8, channel: &mut u32, opmask: &mut u32) -> bool {
...
fn opl_emu_write(emu: &mut OplEmu, regnum: usize, data: u8) {
    ...
    if opl_emu_registers_write(&mut emu.m_regs, regnum, data, &mut keyon_channel, &mut keyon_opmask) {
        if keyon_channel < OPL_EMU_REGISTERS_CHANNELS as u32 {
            ...
            opl_emu_fm_channel_keyonoff(&mut emu.m_channel[6], opl_emu_bitfield(keyon_opmask, 4, 1) * 3, OplEmuKeyonType::Rhythm, 6);
