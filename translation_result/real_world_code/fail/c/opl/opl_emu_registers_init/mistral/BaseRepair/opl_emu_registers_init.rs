...
const S_SIN_TABLE: [u16; 256] = [
    ...
    0x070, 0x06e, 0x06, 0x06a, 0x068, 0x066, 0x064, 0x062, 0x060, 0x05e, 0x05c, 0x05b, 0x059, 0x057, 0x055,
...
fn opl_emu_abs_sin_attenuation(input: u32) -> u16 {
    if (opl_emu_bitfield(input, 8, 1) as u32) != 0 {
        !input as u16
    } else {
        S_SIN_TABLE[input as usize & 0xff]
    }
}
...
fn opl_emu_registers_init(regs: &mut OplEmuRegisters) {
    ...
    let mut wf_iter = regs.m_waveform.iter_mut();
    let wf0 = wf_iter.next().unwrap();
    let wf1 = wf_iter.next().unwrap();
    let wf2 = wf_iter.next().unwrap();
    let wf3 = wf_iter.next().unwrap();
    let wf4 = wf_iter.next().unwrap();
    let wf5 = wf_iter.next().unwrap();
    let wf6 = wf_iter.next().unwrap();
    let wf7 = wf_iter.next().unwrap();

    for index in 0..OPL_EMU_REGISTERS_WAVEFORM_LENGTH {
        let v = opl_emu_abs_sin_attenuation(index as u32) as u16;
        wf0[index] = v | ((opl_emu_bitfield(index as u32, 9, 1) as u16) << 15);
    }
    ...
    if OPL_EMU_REGISTERS_WAVEFORMS >= 4 {
        ...
        for index in 0..OPL_EMU_REGISTERS_WAVEFORM_LENGTH {
            wf1[index] =
                if opl_emu_bitfield(index as u32, 9, 1) != 0 { zeroval } else { wf0[index] };
            wf2[index] = wf0[index] & 0x7fff;
            wf3[index] =
                if opl_emu_bitfield(index as u32, 8, 1) != 0 { zeroval } else { wf0[index] & 0x7fff };
            ...
            if OPL_EMU_REGISTERS_WAVEFORMS >= 8 {
                ...
                wf7[index] =
                    ((opl_emu_bitfield(index as u32, 9, 1) as u16) << 3)
                        .wrapping_shr((15 - 3) as u32);
            }
        }
    }
}
