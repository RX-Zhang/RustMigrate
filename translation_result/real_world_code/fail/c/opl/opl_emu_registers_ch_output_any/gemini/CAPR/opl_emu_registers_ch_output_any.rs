...
fn opl_emu_registers_new(regs: &mut OplEmuRegisters) {
    regs.m_regdata = vec![0; OPL_EMU_REGISTERS_REGISTERS];
    regs.m_waveform = vec![vec![0; OPL_EMU_REGISTERS_WAVEFORM_LENGTH]; OPL_EMU_REGISTERS_WAVEFORMS];
}

fn opl_emu_bitfield(value: u32, start: usize, length: usize) -> u32 {
    (value >> start) & ((1_u32 << length) - 1)
}
...
fn opl_emu_registers_byte(
    ...
    let total_offset = offset.wrapping_add(extra_offset);
    opl_emu_bitfield(regs.m_regdata[total_offset as usize] as u32, start as usize, count as usize)
}
