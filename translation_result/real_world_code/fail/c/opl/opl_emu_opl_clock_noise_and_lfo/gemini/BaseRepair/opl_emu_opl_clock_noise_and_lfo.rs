...
fn opl_emu_opl_clock_noise_and_lfo(
    ...
    *lfo_am_counter = lfo_am_counter.wrapping_add(1);
    if am_counter >= 210u16.wrapping_mul(64).wrapping_sub(1) {
        *lfo_am_counter = 0;
    }

    let shift = 9i32.wrapping_sub(2i32.wrapping_mul(am_depth as i32));
    *lfo_am = ((if am_counter < 105u16.wrapping_mul(64) {
        am_counter
    } else {
        210u16.wrapping_mul(64).wrapping_add(63).wrapping_sub(am_counter)
    }) >> (shift as u16).min(15)) as u8;

    let pm_counter = *lfo_pm_counter;
    *lfo_pm_counter = lfo_pm_counter.wrapping_add(1);

    let pm_scale: [i8; 8] = [8, 4, 0, -4, -8, -4, 0, 4];
    let pm_index = opl_emu_bitfield(pm_counter as u32, 10, 3) as usize;
    let pm_result = pm_scale[pm_index % 8];
    let pm_shift = (pm_depth as i32) ^ 1;
    
    (pm_result as i32).wrapping_shr(pm_shift.min(31) as u32)
}
