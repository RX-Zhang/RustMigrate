...
fn opl_emu_fm_operator_keyonoff(fmop: &mut OplEmuFmOperator, on: u32, keyon_type: OplEmuKeyonType) {
    fmop.m_keyon_live = (fmop.m_keyon_live & !(1 << keyon_type as u8)) | (opl_emu_bitfield(on, 0, 1) << keyon_type as u8) as u8;
}

fn opl_emu_fm_channel_keyonoff(fmch: &mut OplEmuFmChannel, states: u32, keyon_type: OplEmuKeyonType, _chnum: u32) {
    for opnum in 0..fmch.m_op.len() {
        if let Some(ref mut op) = fmch.m_op[opnum] {
            opl_emu_fm_operator_keyonoff(op, opl_emu_bitfield(states, opnum as u8, 1), keyon_type);
        }
    }
}
