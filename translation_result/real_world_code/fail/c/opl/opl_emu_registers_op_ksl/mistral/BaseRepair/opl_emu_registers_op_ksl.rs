...
struct OplEmuRegisters {
    ...
    m_noise_lfsr: u32,
    m_lfo_am: u8,
    m_regdata: [u16; OPL_EMU_REGISTERS_REGISTERS as usize],
...
fn opl_emu_registers_byte(regs: &mut OplEmuRegisters, offset: u32, start: u32, count: u32, extra_offset: u32) -> u32 {
    ...
    if index >= OPL_EMU_REGISTERS_REGISTERS as usize {
        return 0;
    }
...
fn opl_emu_registers_op_ksl(regs: &mut OplEmuRegisters, opoffs: u32) -> u32 {
    let temp = opl_emu_registers_byte(regs, 0x40, 6, 2, opoffs);
    (opl_emu_bitfield(temp, 1, 1) as u32) | ((opl_emu_bitfield(temp, 0, 1) as u32) << 1)
}
