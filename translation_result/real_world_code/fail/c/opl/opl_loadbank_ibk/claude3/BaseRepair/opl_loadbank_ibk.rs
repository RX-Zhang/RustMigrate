...
fn opl_loadbank_internal(opl: &mut Opl, file: &str, offset: i32) -> i32 {
    ...
    if let Err(_) = f.seek(SeekFrom::End(0)) {
        return -2;
    }
    ...
    if let Err(_) = f.read_exact(&mut buff[0..4]) {
        return -3;
    }
    ...
    for i in offset..128 + offset {
        if let Err(_) = f.read_exact(&mut buff) {
            return -4;
        }
