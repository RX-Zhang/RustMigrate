...
fn aptx_invert_quantization(invert_quantize: &mut AptxInvertQuantize, quantized_sample: i32, dither: i32, tables: &AptxTables) {
    let idx = (quantized_sample ^ (! (quantized_sample < 0)) as i32).wrapping_add(1);
    ...
    qr = rshift64_clip24(
        ((qr as i64) * (1i64 << 32)).wrapping_add((dither as i64) * (tables.invert_quantize_dither_factors[idx as usize] as i64)),
        32,
    );
