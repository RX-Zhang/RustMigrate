...
fn clip_intp2(a: i32, p: u32) -> i32 {
    let a_u32 = a as u32;
    if (a_u32.wrapping_add(1u32.wrapping_shl(p))) & !((2u32.wrapping_shl(p)).wrapping_sub(1)) != 0 {
...
fn aptx_qmf_convolution(
    ...
    for i in 0..FILTER_TAPS {
        let index = (signal.pos as usize + i) % (2 * FILTER_TAPS);
        e = e.wrapping_add((signal.buffer[index] as i64).wrapping_mul(coeffs[i] as i64));
    }
