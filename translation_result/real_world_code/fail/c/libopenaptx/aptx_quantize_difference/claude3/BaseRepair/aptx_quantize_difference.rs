...
pub fn aptx_quantize_difference(quantize: &mut AptxQuantize, sample_difference: i32, dither: i32, quantization_factor: i32, tables: &AptxTables) {
    ...
    let d = rshift32_clip24((((dither as i64) * (dither as i64)) >> 32) as i32, 7).wrapping_sub(1 << 23);
    ...
    let dithered_sample = rshift64_clip24((dither as i64) * (interval as i64) + ((clip_intp2(mean.wrapping_add(d), 23) as i64) << 32), 32);
    let error = ((sample_difference_abs as i64) << 20).wrapping_sub((dithered_sample as i64) * (quantization_factor as i64));
